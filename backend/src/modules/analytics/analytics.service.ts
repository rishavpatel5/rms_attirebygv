...
function categoryClause(categoryId?: string | null): Prisma.Sql {
  if (!categoryId) return Prisma.empty;
  return Prisma.sql`AND EXISTS (
    SELECT 1 FROM order_items oi_c
    INNER JOIN product_variants pv_c ON pv_c.id = oi_c.variant_id
    INNER JOIN products pr_c ON pr_c.id = pv_c.product_id
    WHERE oi_c.order_id = o.id AND pr_c.category_id = ${categoryId}
  )`;
}
